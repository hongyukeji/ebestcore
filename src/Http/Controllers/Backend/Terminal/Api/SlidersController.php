...
class SlidersController extends Controller
{
    public function index()
    {
        $pages = [
            ...
            'breadcrumbs' => [
                [
                    ...
                    'icon' => 'fa fa-circle-o',
                    'link' => route('backend.terminal.api.sliders.index'),
                    'active' => true,
                ],
            ],
            'button_group' => [
        ...
        $items = config('terminal.api.sliders');
        return view('backend::terminal.api.sliders.index', compact('items', 'pages'));
    }

    public function store(Request $request)
    {
        $items = $request->except('_token');
        settings(['terminal' => ['api' => ['sliders' => array_replace_recursive(config('terminal.api.sliders'), $items)]]]);
        return redirect()->route('backend.terminal.api.sliders.index')->with('message', trans('backend.messages.update_success'));
    }
}

...
class AdvertsController extends Controller
{
    public function index()
    {
        $pages = [
            ...
            'breadcrumbs' => [
                [
                    ...
                    'icon' => 'fa fa-circle-o',
                    'link' => route('backend.terminal.api.adverts.index'),
                    'active' => true,
                ],
            ],
            'button_group' => [
        ...
        $items = config('terminal.api.adverts');
        return view('backend::terminal.api.adverts.index', compact('items', 'pages'));
    }

    public function store(Request $request)
    {
        $items = $request->except('_token');
        settings(['terminal' => ['api' => ['adverts' => array_replace_recursive(config('terminal.api.adverts'), $items)]]]);
        return redirect()->route('backend.terminal.api.adverts.index')->with('message', trans('backend.messages.update_success'));
    }
}
